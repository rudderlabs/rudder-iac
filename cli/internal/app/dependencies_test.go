...
func TestNewDestinationRegistryFlagMatrix(t *testing.T) {
	...
	cases := []struct {
		...
		},
		{
			...
			destinationSupport:     true,
			unverifiedDestinations: true,
			wantTypes:              []string{"attentive_tag", "bq", "bqstream", "confluent_cloud", "customerio_audience", "facebook_conversions", "gcs", "googlepubsub", "googlesheets", "http", "kinesis", "marketo", "postgres", "redis", "rs", "s3", "s3_datalake", "salesforce", "slack", "snowflake", "statsig", "zendesk"},
		},
	}
