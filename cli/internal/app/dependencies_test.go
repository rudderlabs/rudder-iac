...
func TestNewDestinationRegistryFlagMatrix(t *testing.T) {
	...
	cases := []struct {
		...
		},
		{
			...
			destinationSupport:     true,
			unverifiedDestinations: true,
			wantTypes:              []string{"attentive_tag", "confluent_cloud", "customerio_audience", "facebook_conversions", "gcs", "googlepubsub", "http", "kinesis", "marketo", "rs", "s3", "salesforce", "slack", "statsig", "zendesk"},
		},
	}
