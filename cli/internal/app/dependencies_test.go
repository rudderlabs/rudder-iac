...
func TestNewDestinationRegistryFlagMatrix(t *testing.T) {
	...
	cases := []struct {
		...
		},
		{
			...
			destinationSupport:     true,
			unverifiedDestinations: true,
			wantTypes:              []string{"active_campaign", "adj", "attentive_tag", "bq", "bqstream", "braze", "confluent_cloud", "customerio", "customerio_audience", "facebook_conversions", "ga4", "gcs", "googlepubsub", "googlesheets", "http", "intercom", "kafka", "kinesis", "marketo", "postgres", "posthog", "redis", "rs", "s3", "s3_datalake", "salesforce", "slack", "snowflake", "statsig", "tiktok_ads", "zendesk"},
		},
	}
