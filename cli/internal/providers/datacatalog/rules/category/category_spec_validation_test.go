...
func TestCategorySpecV1SyntaxValidRule_ValidSpecs(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			assert.Empty(t, results, "Valid V1 spec should not produce validation errors")
		})
	}
}
...
func TestCategorySpecV1SyntaxValidRule_InvalidSpecs(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			spec: localcatalog.CategorySpecV1{
				Categories: []localcatalog.CategoryV1{
					{LocalID: "cat1", Name: "Valid Category"},
					{LocalID: "cat2", Name: "Trailing Space "},
					{LocalID: "cat3", Name: "Also Valid"},
				},
			},
			expectedErrors: 1,
