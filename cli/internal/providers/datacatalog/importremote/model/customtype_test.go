...
func TestCustomTypeForExportV0(t *testing.T) {
	t.Run("creates map with externalID set as id", func(t *testing.T) {
		upstream := &catalog.CustomType{
			Name:        "Address",
			Description: "User address information",
		...
		mockRes := &mockResolver{
			resolveFunc: func(entityType string, remoteID string) (string, error) {
				...
				switch remoteID {
				case "prop_street_123":
					return "#/properties/default/street", nil
				case "prop_city_456":
					return "#/properties/default/city", nil
				case "prop_zip_789":
					return "#/properties/default/zip_code", nil
				default:
					return "", fmt.Errorf("unknown property: %s", remoteID)
				}
			},
		...
		assert.Equal(t, map[string]any{
			...
			"properties": []any{
				map[string]any{
					"$ref":     "#/properties/default/street",
					"required": true,
				},
				map[string]any{
					"$ref":     "#/properties/default/city",
					"required": true,
				},
				map[string]any{
					"$ref":     "#/properties/default/zip_code",
					"required": false,
				},
			},
		}, result)
	...
	t.Run("resolves itemTypes custom type references in config", func(t *testing.T) {
		customTypeID := "ct_product_123"
		expectedRef := "#/custom-types/default/Product"
	...
	t.Run("resolves variant with discriminator and cases", func(t *testing.T) {
		...
		mockRes := &mockResolver{
			resolveFunc: func(entityType string, remoteID string) (string, error) {
				assert.Equal(t, types.PropertyResourceType, entityType)
				refMap := map[string]string{
					"prop_page":        "#/properties/default/page",
					"prop_search_term": "#/properties/default/search_term",
					"prop_product_id":  "#/properties/default/original_product_id",
				}
				if ref, ok := refMap[remoteID]; ok {
					return ref, nil
				}
		...
		expected := map[string]any{
			...
			"properties": []any{
				map[string]any{
					"$ref":     "#/properties/default/page",
					"required": true,
				},
				map[string]any{
					"$ref":     "#/properties/default/search_term",
					"required": false,
				},
				map[string]any{
					"$ref":     "#/properties/default/original_product_id",
					"required": false,
				},
			},
			"variants": []any{
				map[string]any{
					"type":          "discriminator",
					"discriminator": "#/properties/default/page",
					"cases": []any{
						map[string]any{
							"display_name": "search_page",
							"match":        []any{"search", "search_bar"},
	...
	t.Run("errors when variant case property resolver fails", func(t *testing.T) {
		...
		mockRes := &mockResolver{
			resolveFunc: func(entityType string, remoteID string) (string, error) {
				if remoteID == "prop_discriminator" {
					return "#/properties/default/discriminator", nil
				}
				return "", fmt.Errorf("case property not found")
			},
		}
