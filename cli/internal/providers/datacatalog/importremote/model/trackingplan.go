...
func (tp *ImportableTrackingPlanV1) fromUpstream(
	...
	v0TrackingPlan := ImportableTrackingPlan{}
	if err := v0TrackingPlan.fromUpstream(externalID, upstream, resolver, idNamer); err != nil {
		return fmt.Errorf("loading tracking plan from upstream: %w", err)
	}

	err := tp.TrackingPlanV1.FromV0(&v0TrackingPlan.TrackingPlan)
	if err != nil {
