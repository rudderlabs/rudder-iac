...
// SQLModelHandler implements the resourceHandler interface for SQL Model resources
type SQLModelHandler struct {
	client retlClient.RETLStore
	specs  []*SQLModelSpec
}

// NewSQLModelHandler creates a new SQL Model resource handler
func NewSQLModelHandler(client retlClient.RETLStore) *SQLModelHandler {
	return &SQLModelHandler{
		client: client,
		specs:  []*SQLModelSpec{},
	}
}
...
func (h *SQLModelHandler) LoadSpec(path string, s *specs.Spec) error {
	spec := &SQLModelSpec{}
...
func (h *SQLModelHandler) Validate() error {
...
func (h *SQLModelHandler) GetResources() ([]*resources.Resource, error) {
	...
	for _, spec := range h.specs {
		...
		// Add file or SQL content if present
		if spec.File != nil {
			data["file"] = *spec.File
		}
		if spec.SQL != nil {
		...
		// Create resource with "sql-model" type
...
func (h *SQLModelHandler) Create(ctx context.Context, ID string, data resources.ResourceData) (*resources.ResourceData, error) {
...
func (h *SQLModelHandler) Update(ctx context.Context, ID string, data resources.ResourceData, state resources.ResourceData) (*resources.ResourceData, error) {
...
func (h *SQLModelHandler) Delete(ctx context.Context, ID string, state resources.ResourceData) error {
	// Get source_id from state - needed for API call
	sourceID, ok := state["source_id"].(string)
	if !ok {
