...
func TestAccountReference(t *testing.T) {
	...
	t.Run("becomes a dependency on the account that resolves to its remote id", func(t *testing.T) {
		...
		ref, ok := r.Data()[sourcekeys.AccountIDKey].(*resources.PropertyRef)
		require.True(t, ok, "account_id holds a reference, got %T", r.Data()[sourcekeys.AccountIDKey])
		...
		assert.Equal(t, "acc-remote", data[sourcekeys.AccountIDKey])

		output, err := h.Create(context.Background(), r.ID(), data)
		require.NoError(t, err)
		assert.Equal(t, "acc-remote", store.sources[(*output)[sourcekeys.IDKey].(string)].AccountID)
	})
...
func TestAccountStateFollowsSpecForm(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			created[sourcekeys.AccountIDKey] = "acc-remote"
			...
			assert.Contains(t, diffs, sourcekeys.AccountIDKey)
		})
	}
}
