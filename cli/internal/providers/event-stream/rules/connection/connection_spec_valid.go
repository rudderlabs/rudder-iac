...
// id side deliberately accepts anything non-empty — endpoint local ids carry
// no charset restriction and the handler's parser is equally lenient, so a
// stricter pattern here could reject a spec that would apply cleanly.
