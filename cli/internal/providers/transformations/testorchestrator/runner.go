...
func (r *Runner) Run(ctx context.Context, mode Mode, targetID string) (*TestResults, error) {
	...
	for urn, resource := range remoteState.Resources {
		fmt.Println("Remote resource", "urn", urn, "type", resource.Type, "id", resource.ID)
	}

	for urn, resource := range remoteResources.GetAll("transformation-library") {
		fmt.Println("Remote resource", "urn", urn, "externalId", resource.ExternalID, "id", resource.ID)
	}
