...
// The Marketo native-SDK toggle is deliberately absent. schema.json declares it
// as a plain boolean, but db-config lists it nowhere — not in defaultConfig and
// not under any source type — so the upstream sources disagree on whether the
// key exists. Settle it in integrations-config before modelling it here (DEX-721).
//
