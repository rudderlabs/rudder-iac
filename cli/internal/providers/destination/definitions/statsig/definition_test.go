...
func TestStatsigConfigValidation(t *testing.T) {
	...
	t.Run("secret_key too long", func(t *testing.T) {
		t.Parallel()
		errors := registered.ValidateConfig(map[string]any{
			"secret_key": strings.Repeat("a", 201),
		})
		require.Len(t, errors, 1)
		assert.Equal(t, "/secret_key", errors[0].Path)
		assert.Contains(t, errors[0].Message, "at most 200 characters")
	})

	// `^(.{1,200})$` is a pattern, not a length limit: it forbids line breaks,
	// and the template branch upstream carries no length cap.
	t.Run("secret_key rejects line breaks", func(t *testing.T) {
		t.Parallel()
		errors := registered.ValidateConfig(map[string]any{"secret_key": "line\nbreak"})
		require.Len(t, errors, 1)
		assert.Equal(t, "/secret_key", errors[0].Path)
	})

	t.Run("secret_key accepts ui templates regardless of length", func(t *testing.T) {
		t.Parallel()
		long := "{{ config.secretKey || " + strings.Repeat("x", 300) + " }}"
		require.Greater(t, len(long), 200)
		assert.Empty(t, registered.ValidateConfig(map[string]any{"secret_key": long}))
	})

	t.Run("deprecated env references get no template exemption", func(t *testing.T) {
		t.Parallel()
		errors := registered.ValidateConfig(map[string]any{
			"secret_key": "env." + strings.Repeat("A", 201),
		})
		require.Len(t, errors, 1)
		assert.Equal(t, "/secret_key", errors[0].Path)
	})
