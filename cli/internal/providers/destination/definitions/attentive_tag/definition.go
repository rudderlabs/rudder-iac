...
const signUpSourceIDPattern = `^[0-9]*$`

func init() {
	// Strict digits-only — no env./{{ }} escape hatch (unlike terraform / schema.json).
	funcs.NewPattern(
		"attentive_tag_sign_up_source_id",
		signUpSourceIDPattern,
...
func NewDefinition() *definitions.DestinationDefinition {
	properties := []converter.ConfigProperty{
		...
		converter.Simple("signUpSourceId", "sign_up_source_id"),
		converter.Simple("enableNewIdentifyFlow", "enable_new_identify_flow"),
	}
