...
func TestNewDefinitionMetadata(t *testing.T) {
	...
	assert.Equal(t, []string{"dsn"}, registered.SecretKeys())
	assert.Equal(t, []string{"dsn"}, registered.ReturnedSecretKeys())
...
func TestSentryConfigValidation(t *testing.T) {
	...
	t.Run("dsn rejects line break", func(t *testing.T) {
		t.Parallel()
		errors := registered.ValidateConfig(map[string]any{
			"dsn": "https://public@o0.ingest.sentry.io\n/0",
		})
		require.Len(t, errors, 1)
		assert.Equal(t, "/dsn", errors[0].Path)
	})

	t.Run("dsn accepts template", func(t *testing.T) {
		t.Parallel()
		errors := registered.ValidateConfig(map[string]any{
			"dsn": "{{ .sentry.dsn || \"https://public@o0.ingest.sentry.io/0\" }}",
		})
		assert.Empty(t, errors)
	})

	t.Run("allow_urls accepts template", func(t *testing.T) {
		t.Parallel()
		errors := registered.ValidateConfig(map[string]any{
			"dsn":        "https://public@o0.ingest.sentry.io/0",
			"allow_urls": []any{"{{ .sentry.allowUrl || \"https://app.example.com\" }}"},
		})
		assert.Empty(t, errors)
	})
