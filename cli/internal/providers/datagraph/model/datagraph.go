...
// match the rudder-api column-metadata contract: length-bounded (255 for name
// and display_name, 2000 for description), trimmed, no control characters, and
// (for display_name) case-insensitively unique within a model. Each entry must
// set at least one of display_name, description, or pii_mask — that cross-field
// rule is enforced by the spec validator, not struct tags.
type ColumnMetadataYAML struct {
	...
	Description string `json:"description,omitempty" yaml:"description,omitempty" mapstructure:"description" validate:"omitempty,max=2000"`
