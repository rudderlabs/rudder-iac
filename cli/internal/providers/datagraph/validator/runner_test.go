...
func TestResolveAccountIDs_MissingDataGraphRef(t *testing.T) {
	graph := resources.NewGraph()
	runner := &Runner{graph: graph}
	...
	err := runner.resolveAccountIDs(plan)
	require.Error(t, err)
	assert.Contains(t, err.Error(), "could not determine data graph for resource data-graph-model:user")
}
