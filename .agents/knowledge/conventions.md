...
## RUD-11 — CLI Metadata Output Placement
- For user-visible run metadata (like workspace identity), prefer reusing data already fetched in the command orchestration layer and printing via shared UI message helpers, instead of adding provider/control-plane fetches solely for display.
- Use the common workspace print format `Workspace: <name> (ID: <id>)` so apply/workspace-facing output stays consistent across command flows.
