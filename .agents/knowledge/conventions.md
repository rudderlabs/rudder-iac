...
## RUD-20 — README Rewrite Guardrails
<!-- ticket:RUD-20 -->
- README edits should preserve explicit onboarding paths for all supported workflows: release binary install, Docker usage, and local build-from-source.
- Concision rewrites are acceptable when identity remains explicit (`rudder-iac` repository and `rudder-cli` binary) and at least one concrete command path remains for each workflow.
