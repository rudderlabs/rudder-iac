...
## RUD-11 — Apply Workspace Identity Reporting
- `apply` resolves workspace context via `Workspaces.GetByAuthToken` before sync planning/execution, and now emits workspace identity immediately (`Workspace: <name> (ID: <id>)`) through `ui.PrintWorkspace`; this keeps operator context visible in both normal apply and `--dry-run` paths.
- The fetch-and-print happens at command level (project apply command) rather than provider internals, preserving provider boundaries while making run-scoped metadata consistently available to CLI users.
