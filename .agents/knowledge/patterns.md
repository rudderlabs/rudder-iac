...
## DEX-888 — Destination Returned Secret Keys
<!-- ticket:DEX-888 -->
- Destination definitions distinguish `SecretKeys` (all local config paths that must be redacted/masked and revealed only for writes) from `ReturnedSecretKeys` (the subset of secrets the destination API returns in read responses).
- `HandlerImpl.MapRemoteToState` wraps returned secrets with known `secret.String` values for stable diffing, and wraps only `WriteOnlySecretKeys()` as unknown; use this for secrecy-policy exceptions where the API still returns a value.
- DEX-888 marks API-returned local fields as destination secrets despite current `db-config.secretKeys` omissions: Adjust `app_token`, and ActiveCampaign `actid` alongside existing `api_key` and `event_key`.
- Destination E2E fixtures should use variable placeholders for newly secret fields, command-output raw-secret checks should include the resolved fixture values, and upstream snapshots should use `<ignored>` plus comparator ignore paths such as `config.appToken` or `config.actid` instead of committing API-returned secrets.
