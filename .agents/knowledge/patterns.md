...
## DEX-527 — Snowpipe Streaming E2E Runs; `namespace` Is Immutable
<!-- ticket:DEX-527 -->
- Snowpipe Streaming **is** runnable in `TestDestinationsApply`. An earlier note claimed the `SNOWFLAKE_STREAMING` gate (`options.hidden.gate.flags` in db-config) forces a `403` at create; a live create on the e2e workspace succeeded, so the gate did not block it there. Verify a gate empirically before deferring on it — `options.hidden.gate` is a UI-visibility hint and is not necessarily enforced by the destinations API.
...
- `privateKey` and `privateKeyPassphrase` are write-only and correctly absent from upstream snapshots.
