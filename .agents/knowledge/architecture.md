...
## DEX-999 — Release Rule Catalog Publication

- `.github/workflows/publish-rules-to-hugo.yml` publishes validation-rule documentation from the exact release tag on `repository_dispatch: release-created`, with a manual tag-based `workflow_dispatch` path for dry runs; this avoids relying on `release: published`, which release-please events created with `GITHUB_TOKEN` do not trigger.
- The workflow injects the checked-out release version into the rule-doc generator through Go ldflags, verifies that the catalog does not regress below the 38-rule baseline, and uploads `rules.yaml` as an idempotently replaced GitHub release asset.
- Downstream Hugo publication uses a short-lived GitHub App token scoped to `rudder-hugo` to create or update `auto/rudder-cli-rules-<version>`, changing `data/rudder-cli-rules.raw.yaml`; the repository `GITHUB_TOKEN` retains only job-scoped `contents: write` for the release asset.
