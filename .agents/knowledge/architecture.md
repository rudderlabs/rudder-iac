...
- HTTP is treated as an unverified destination: it should register only when `ExperimentalFlags.UnverifiedDestinations` is enabled; destination support itself is now GA. S3 and Attentive Tag are verified/native destinations and do not share that gate.
...
<!-- ticket:DEX-608 -->
- S3 and Attentive Tag are verified/native destination definitions: `cli/internal/app/newDestinationRegistry` should register `s3.NewDefinition()` and `attentivetag.NewDefinition()` unconditionally, without requiring `ExperimentalFlags.UnverifiedDestinations`.
...
- Destination registry flag-matrix expectations should reflect S3 and Attentive Tag as always available, while enabling unverified destinations adds HTTP alongside those verified definitions.
- This supersedes earlier destination-registry guidance that used S3 as an unverified peer example for HTTP; HTTP remains gated, but S3 and Attentive Tag no longer share that gate.
...
- S3 and Attentive Tag remain the verified/native destinations registered unconditionally; do not use their gate as the GCS registry precedent.
...
- Every verified definition whose db-config lists `warehouse` now declares it with its full rETL metadata. Unverified eligible definitions are deferred, and the backfill promotes nothing to verified. `.claude/skills/onboard-cli-destination/reference/source-type-mapping.md` "rETL metadata" holds the derivation and the commands that reproduce it.
- Declaring `warehouse` also maps `connectionMode.warehouse` and `consentManagement.warehouse` in both conversion directions. The webapp writes `connectionMode.<sourceType>` for every connected source type, so a UI-made destination that already has an rETL connection carries these keys. A spec imported before the backfill lacks them: after upgrading, `plan` shows an update removing them, and `apply`, which sends the full config, strips them (and any warehouse consent settings) from a destination nobody edited. Re-import those destinations after upgrading.

## DEX-1012 — Stable Schema URLs and Editor Association

- Arbitrarily named Rudder YAML specs should use a per-file `yaml-language-server` schema modeline as the primary portable association mechanism. VS Code/JetBrains project mappings and future extension content detection are bulk alternatives; a narrow `*.rudder.yaml` SchemaStore entry is only additive discovery because `fileMatch` cannot inspect content and broad globs would misclassify unrelated YAML.
- Canonical schemas should be published under the product-owned, spec-versioned namespace `https://www.rudderstack.com/docs/schemas/rudder-cli/v1/`. This tracks the `rudder/v1` contract independently of CLI releases and permits controlled CORS, caching, content type, redirects, and retention of old schema versions; release bundles remain the immutable/offline fallback.
- Publish both `rudder-spec.schema.json` and each `<kind>.schema.json`. Importer and scaffolding writers already know the document kind, so their modelines must use `schemaBaseURL + "/" + <kind> + ".schema.json"`; reserve the combined root for SchemaStore and project mappings spanning multiple kinds.
- `docs/spikes/DEX-1012/demo/` contains the root and event-stream-source schemas generated by rudder-iac PR #927 commit `bbcee89`. The root has 14 constant-`kind` `oneOf` branches. With `yaml-language-server` 1.24.0, completion inside `spec` before `kind` mixes fields from multiple branches; after `kind` is present, completion narrows correctly and hover identifies the selected schema.
- Treat the generated PR #927 bundle in the spike as reproducible evidence from an open dependency, not as a published schema contract. Part 2 must regenerate from the merged DEX-1000/DEX-1004 source before hosting the files.
