...
- Publish both a kind-discriminated root schema and per-kind schemas. The default modeline should target the root for one durable URL and completion when changing `kind`; generated/exported files may target per-kind URLs for stronger diagnostics before `kind` is present.
- `yaml-language-server` 1.24.0 demonstrated kind-specific completion, hover, and diagnostics through Draft 7 `if`/`then` branches when each `if` also requires `kind`. Missing `kind` intentionally yields envelope-only completion plus a missing-kind diagnostic.
- The hand-written event-stream-source and data-graph demo schemas are spike evidence only and must not be treated as the generated DEX-1000 schema contract.
