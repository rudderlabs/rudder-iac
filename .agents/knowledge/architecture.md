...
- The CLI owns explicit visible `help` and `completion` commands so runtime, generation, and completeness checks share the same surface. Cobra's default completion command is disabled, and its internal help command is hidden under `__help` to avoid duplicate generated entries.
