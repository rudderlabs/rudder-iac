...
- CI failed when `rudder-cli migrate` validated substituted specs but ran provider migrations on raw parsed legacy specs, causing legacy references like `#/properties/...` to be written into rudder/v1 files and fail apply dry-run syntax validation.
- Durable mitigation: preserve placeholder writes by migrating original/raw specs, but ensure DataCatalog migration converts legacy path refs before extraction so output uses `#property:`, `#event:`, `#category:`, and `#custom-type:` forms without serializing resolved substitution values.
